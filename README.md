...
- **PHP 版本**: 支持 PHP 8.2 和 8.3。正在计划适配 PHP 8.4 和 8.5。
...
> **⚠️ 注意**：OpKit 与 `Zend OPcache` 存在底层机制冲突，**两者不能同时开启**。请确保已禁用 `extension=opcache.so` 或 `zend_extension=opcache.so`。
