...
{
	opkit_persistent_script *persistent_script = (opkit_persistent_script *) emalloc(sizeof(opkit_persistent_script));
	memset(persistent_script, 0, sizeof(opkit_persistent_script));

	zend_hash_init(&persistent_script->script.script.function_table, 0, NULL, ZEND_FUNCTION_DTOR, 0);
	 ...
	 * PHP compiler
	 */
	zend_hash_init(&persistent_script->script.script.class_table, 0, NULL, ZEND_CLASS_DTOR, 0);
	zend_hash_init(&persistent_script->constants_table, 0, NULL, opkit_free_zend_constant, 0);

	return (zend_persistent_script *)persistent_script;
}

void free_persistent_script(zend_persistent_script *persistent_script, int destroy_elements)
{
	opkit_persistent_script *op_script = (opkit_persistent_script *)persistent_script;
	if (!destroy_elements) {
		/* Both the keys and values have been transferred into the global tables or persisted.
		...
		op_script->constants_table.nNumUsed = 0;

		/* Ensure we don't free arData if it was changed to point into the persisted memory block */
		HT_SET_DATA_ADDR(&persistent_script->script.function_table, NULL);
		HT_FLAGS(&persistent_script->script.function_table) |= HASH_FLAG_UNINITIALIZED;
		HT_SET_DATA_ADDR(&persistent_script->script.class_table, NULL);
		HT_FLAGS(&persistent_script->script.class_table) |= HASH_FLAG_UNINITIALIZED;
		HT_SET_DATA_ADDR(&op_script->constants_table, NULL);
		HT_FLAGS(&op_script->constants_table) |= HASH_FLAG_UNINITIALIZED;
	} else {
...
void zend_accel_move_user_constants(HashTable *src, uint32_t count, zend_persistent_script *script)
{
	Bucket *p, *end;
	HashTable *dst;
	dtor_func_t orig_dtor;
	...
	dst = &((opkit_persistent_script *)script)->constants_table;
	orig_dtor = src->pDestructor;
	src->pDestructor = NULL;
	zend_hash_extend(dst, count, 0);
	end = src->arData + src->nNumUsed;
	...
	for (; p != end; p++) {
		if (UNEXPECTED(Z_TYPE(p->val) == IS_UNDEF)) continue;
 	zc = Z_PTR(p->val);
			/* Check if constant belongs to the currently compiled file */
		if (zc->name && (Z_CONSTANT_FLAGS(zc->value) & CONST_OWNED_BY_PERSISTENT_SCRIPT)) {
			_zend_hash_append_ptr(dst, p->key, zc);
			zend_hash_del_bucket(src, p);
		}
	}
	src->pDestructor = orig_dtor;
...
void zend_accel_build_delayed_early_binding_list(zend_persistent_script *persistent_script)
{
	...
	for (zend_op *opline = op_array->opcodes; opline < end; opline++) {
		if (opline->opcode == ZEND_DECLARE_CLASS_DELAYED) {
			...
			zval *parent_name_zv = RT_CONSTANT(opline, opline->op2);
			early_binding->lc_parent_name = (Z_TYPE_P(parent_name_zv) == IS_STRING) ? zend_string_copy(Z_STR_P(parent_name_zv)) : NULL;
...
void zend_accel_do_delayed_early_binding(
		zend_persistent_script *persistent_script, zend_op_array *op_array)
{
	void *run_time_cache = ZEND_MAP_PTR(op_array->run_time_cache);
	if (!run_time_cache) return;
...
void zend_accel_constant_hash_copy(HashTable *target, HashTable *source)
{
	Bucket *p, *end;
	zval *t;
	...
	for (; p != end; p++) {
		...
		t = zend_hash_find_known_hash(target, p->key);
		if (UNEXPECTED(t != NULL)) {
			zend_constant *c2 = Z_PTR_P(t);
			zend_error(E_NOTICE, "Constant %s already defined (previously declared in %s:%d)",
				ZSTR_VAL(p->key),
				c2->name ? "unknown" : "internal",
				0);
			continue;
		}
		_zend_hash_append_ptr_ex(target, p->key, Z_PTR(p->val), 1);
