...
#define ADD_SIZE(m)        ADD_SIZE_MS(m)

#undef ADD_SIZE_MD
#define ADD_SIZE_MD(m)     do { size_t __s = ZEND_ALIGNED_SIZE(m); ZCG(current_persistent_script)->size += __s; opkit_metadata_size += __s; } while(0)
#undef ADD_SIZE_CD
#define ADD_SIZE_CD(m)     do { size_t __s = ZEND_ALIGNED_SIZE(m); ZCG(current_persistent_script)->size += __s; opkit_code_size += __s; } while(0)
#undef ADD_SIZE_DT
#define ADD_SIZE_DT(m)     do { size_t __s = ZEND_ALIGNED_SIZE(m); ZCG(current_persistent_script)->size += __s; opkit_data_size += __s; } while(0)
#undef ADD_SIZE_MS
#define ADD_SIZE_MS(m)     do { size_t __s = ZEND_ALIGNED_SIZE(m); ZCG(current_persistent_script)->size += __s; opkit_misc_size += __s; } while(0)
#define ADD_SIZE_DATA(m)   ADD_SIZE_DT(m)

#define ADD_STRING(str) do { \
		if (str) { \
			if (!zend_shared_alloc_get_xlat_entry(str)) { \
				size_t __s = ZEND_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(ZSTR_LEN(str))); \
				ZCG(current_persistent_script)->size += __s; \
				opkit_data_size += __s; \
				zend_shared_alloc_register_xlat_entry(str, str); \
			} \
		} \
	} while (0)
...
static void zend_persist_ast_calc(zend_ast *ast)
{
	...
	if (ast->kind == ZEND_AST_ZVAL || ast->kind == ZEND_AST_CONSTANT) {
		...
		ADD_SIZE_MS(sizeof(zend_ast_list) - sizeof(zend_ast *) + sizeof(zend_ast *) * list->children);
...
static void zend_persist_op_array_calc_ex(zend_op_array *op_array)
{
	...
	if (op_array->literals) {
		...
		ADD_SIZE_DT(sizeof(zval) * op_array->last_literal);
		while (p < end) {
			if (Z_TYPE_P(p) == IS_STRING) {
				ADD_INTERNED_STRING(Z_STR_P(p));
			} else {
	...
	if (op_array->arg_info) {
		...
		ADD_SIZE_DT(sizeof(zend_arg_info) * num_args);
	...
	if (op_array->cache_size) {
		ADD_SIZE_MD(op_array->cache_size);
	}

	if (op_array->vars) {
		int i;

		ADD_SIZE_DT(sizeof(zend_string*) * op_array->last_var);
...
static void zend_accel_persist_constant_table_calc(HashTable *constants_table)
{
	Bucket *p;

	zend_hash_persist_calc(constants_table);
	ZEND_HASH_MAP_FOREACH_BUCKET(constants_table, p) {
		ZEND_ASSERT(p->key != NULL);
		ADD_INTERNED_STRING(p->key);
		ADD_SIZE(sizeof(zend_constant));
		zend_constant *zc = Z_PTR(p->val);
		if (zc->name) {
			ADD_INTERNED_STRING(zc->name);
		}
		zend_persist_zval_calc(&zc->value);
	} ZEND_HASH_FOREACH_END();
}
...
uint32_t zend_accel_script_persist_calc(zend_persistent_script *script, int for_shm)
{
	...
	zend_persist_warnings_calc(
		script->num_warnings, script->warnings);
